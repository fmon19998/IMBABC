...
export default function DashboardPage(){
  ...
  const [orgName,setOrgName]=useState("");const [company,setCompany]=useState("");
  ...
  async function apiRequest<T>(url:string,body?:object):Promise<T>{
    ...
    if(!session?.access_token)throw new Error("Silakan masuk kembali ke akun Anda");
    const response=await fetch(url,{method:body?"POST":"GET",
      headers:{authorization:"Bearer "+session.access_token,...(body?{"content-type":"application/json"}:{})},
      ...(body?{body:JSON.stringify(body)}:{})});
    const payload=await response.json();
    if(!response.ok)throw new Error((payload as {error?:string}).error||"Permintaan gagal");
    return payload as T;
  }
  useEffect(()=>{
    let live=true;
    async function start(){
      ...
      setCompany(String(user.user_metadata?.company_name||""));
  ...
  async function createWorkspace(e:FormEvent){
    e.preventDefault();const db=getSupabase();if(!db||!userId)return;
    const value=company.trim();if(value.length<2)return;
    setBusy(true);setError("");
    const {data,error:orgError}=await db.from("organizations").insert({name:value,owner_id:userId}).select("id,name").single();
    if(orgError||!data){setError(orgError?.message||"Workspace belum dibuat.");setBusy(false);return}
    const {error:workspaceError}=await db.from("workspaces").insert({organization_id:data.id,name:"Utama",created_by:userId});
    const {error:memberError}=await db.from("organization_members").insert({organization_id:data.id,user_id:userId,role:"OWNER"});
    setBusy(false);
    if(workspaceError||memberError){setError("Organisasi dibuat, tetapi pengaturan awal belum lengkap: "+(workspaceError?.message||memberError?.message));return}
    setOrgId(data.id);setOrgName(data.name);setNotice("Workspace berhasil dibuat.");
  }
  ...
  async function launchCampaign(id:string){
    ...
    try{const result=await apiRequest<{queued:number}>("/api/campaigns/launch",{
      organizationId:orgId,campaignId:id,templateId});
      setNotice(`${result.queued} penerima berizin masuk antrean. Worker akan mengirim lewat WhatsApp Cloud API.`);
  ...
  async function signOut(){const db=getSupabase();await db?.auth.signOut();router.replace("/login")}
