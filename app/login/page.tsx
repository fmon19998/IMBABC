...
export default function Login({portal="AGENT"}:{portal?:"AGENT"|"SUPER_ADMIN"}) {
  ...
  async function submit(e:FormEvent) {
    ...
    const {error}=await db.auth.signInWithPassword({email:portal==="SUPER_ADMIN"&&!email.includes("@")?`${email.trim().toLowerCase()}@admin.imbabc.internal`:email.trim(),password}); setBusy(false);
  ...
  return <main className="auth-shell"><div className="auth-panel">
    ...
    <form onSubmit={submit}><label htmlFor="email">{portal==="SUPER_ADMIN"?"ID super admin":"Email"}</label><Input id="email" type={portal==="SUPER_ADMIN"?"text":"email"} autoComplete="username" required value={email} onChange={e=>setEmail(e.target.value)} placeholder={portal==="SUPER_ADMIN"?"ID super admin":"nama@perusahaan.com"}/>
    ...
    <p className="auth-bottom">{portal==="SUPER_ADMIN"?<>Akses khusus pemilik IMBABC.</>:<>Akun agen dibuat oleh super admin. <Link href="/admin/login">Portal super admin</Link></>}</p>
